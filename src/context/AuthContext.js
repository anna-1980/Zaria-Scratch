  ...
  //to register a new user
    ...
    const newProject = async  formData  => {
     ...
     const deleteProject = async  (id, adminId)  => {
      ...
      try {
        setLoading(true); 
        const { data }= await axios.delete(`${process.env.REACT_APP_API}/api/projects/${id}`, adminId,
        { headers: { Authorization: token}});
        console.log(data);
        setLoading(false); 
        setUploaded(true);
     ...
     const newPicture = async  formData  => {
      try {
        setLoading(true); 
        const { data }= await axios.post(`${process.env.REACT_APP_API}/api/uploads`, formData,
        { headers: { Authorization: token}});
        console.log(data);
        setLoading(false); 
        setUploaded(true);
    ...
    return <AuthContext.Provider value={{isAuthenticated, signup, signin, signout , user, loading, token, isAdmin, newProject, newPicture, deleteProject, uploaded}}>{children}</AuthContext.Provider>
